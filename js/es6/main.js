(function() {
  ...
  function doAdd() {
    doShow();
    var location = $('#loc').val().trim();
    var url = 'http://api.wunderground.com/api/8602561c1a1ca88c/forecast10day/q/' + location + '.json?callback=?';
    $.getJSON(url, addLocation);
  }
  function addLocation(cond) {
    var days = cond.forecast.simpleforecast.forecastday;
    var zip = $('#loc').val().trim();
    $('#loc').val('');
    $('#loc').focus();
